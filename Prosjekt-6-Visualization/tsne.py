...
class TSNE():
    ...
    def execute_algorithm(self):
        ...
        big_p_graph = p_graph / np.sum(p_graph)
        ...
        for dim in range(2):
            for i in range(rows):
                y_values[i][dim] = random.gauss(0, 0.0001)
        ...
        for iteration in range(5):
            ...
            q_values = 1 / (1 + (vector + vector.T -
                            2 * (y_values @ y_values.T)))

            big_q_values = q_values / np.sum(q_values)

            big_g = 4 * (big_p_graph - big_q_values) * q_values
            big_s = np.diag(np.sum(big_g, axis=1))

            # calculating the gradient over each y_i
            gradient_matrix = 4 * (big_s - big_g) @ y_values
